...
def iterative_training_with_lof(X_train, y_train, model, n_iterations=5,):
    ...
    lof_scores, _ = LOF(X_train, k=5, metric='euclidean', contamination='auto')
    ...
    for iteration in range(n_iterations):
        print(f"\nIteracja {iteration + 1}/{n_iterations}")

        # Wybór próbek dla tej iteracji
        n_samples_current = batch_sizes[iteration]
        current_indices = sorted_indices[:n_samples_current]
